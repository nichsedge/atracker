      ...
      <div className="modal-content">
        ...
        <form onSubmit={handleSubmit} className="manual-form" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div className="form-group">
            <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-secondary)', marginBottom: '0.5rem', display: 'block' }}>Rule Type</label>
            <select 
              value={formData.rule_type}
              onChange={e => setFormData({...formData, rule_type: e.target.value})}
              style={{ width: '100%', padding: '0.75rem', borderRadius: '10px', border: '1px solid var(--border-color)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: '1rem' }}
            >
              <option value="ignore" style={{ background: 'var(--bg-main)', color: 'var(--text-primary)' }}>Ignore (Don't track at all)</option>
              <option value="redact" style={{ background: 'var(--bg-main)', color: 'var(--text-primary)' }}>Redact (Track app but hide title)</option>
            </select>
          </div>
          <div className="form-group">
            ...
            <input 
              ...
              style={{ width: '100%', padding: '0.75rem', borderRadius: '10px', border: '1px solid var(--border-color)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: '1rem', fontFamily: 'monospace' }}
            />
          </div>
          <div className="form-group">
