...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        // Migration v3 -> v4: add indices on startTimestamp, synced, packageName
        private val MIGRATION_3_4 = object : Migration(3, 4) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("CREATE INDEX IF NOT EXISTS index_events_startTimestamp ON events(startTimestamp)")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_events_synced ON events(synced)")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_events_packageName ON events(packageName)")
            }
        }
